...
nebula::cpp2::ErrorCode MetaJobExecutor::execute() {
  ...
  std::vector<folly::SemiFuture<Status>> futureStatus;
  for (auto& address : addresses) {
    // transform to the admin host
    auto h = Utils::getAdminAddrFromStoreAddr(address.first);
    futureStatus.emplace_back(executeInternal(std::move(h), std::move(address.second)));
  }

  auto rc = nebula::cpp2::ErrorCode::SUCCEEDED;
  auto tries = folly::collectAll(std::move(futureStatus)).get();
  for (auto& t : tries) {
    if (t.hasException()) {
